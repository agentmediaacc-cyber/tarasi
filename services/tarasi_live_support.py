...
def _support_chat_table_missing(exc: Exception) -> bool:
    message = str(exc or "").lower()
    return 'relation "tarasi_support_chats" does not exist' in message
...
def _next_chat_number() -> str:
    ...
    try:
        ...
        if _support_chat_table_missing(e):
            print("[WARN] tarasi_support_chats table is missing; using fallback chat counter.")
        else:
            print(f"[ERROR] _next_chat_number error: {e}")
...
def get_open_support_chats() -> list[dict[str, Any]]:
    ...
    try:
        ...
        print(f"[ERROR] get_open_support_chats error: {e}")
        return []
...
def get_support_chat(chat_number: str) -> dict[str, Any] | None:
    ...
    try:
        ...
        print(f"[ERROR] get_support_chat error: {e}")
        return None
...
def get_active_chat_for_session(session_id: str) -> dict[str, Any] | None:
    ...
    if mode == "postgres":
        try:
            ...
            if _support_chat_table_missing(e):
                print("[WARN] tarasi_support_chats table is missing; returning no active chat.")
            else:
                print(f"[ERROR] get_active_chat_for_session postgres error: {e}")
            return None
    elif mode == "supabase":
        try:
            ...
            print(f"[ERROR] get_active_chat_for_session supabase error: {e}")
            return None
    else:
        try:
            ...
            print(f"[ERROR] get_active_chat_for_session json error: {e}")
            return None
