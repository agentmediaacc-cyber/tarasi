...
def get_dashboard_summary() -> dict[str, Any]:
    # Use smaller limits for quick summary
    ...
    # Count waiting support chats efficiently
    waiting_support = 0
    mode = _db_mode()
    if mode == "postgres":
        try:
    ...
    # Enrich only the latest 5 conversations instead of 8 to save time
    for row in conversations[:5]:
        # Only fetch last 2 messages for quick context
        messages = get_conversation_messages(row.get("id", ""), limit=2)
        last_user = next((m for m in reversed(messages) if m.get("sender") == "user"), {})
        last_bot = next((m for m in reversed(messages) if m.get("sender") == "bot"), {})
        
        user_key = str(row.get("user_id") or row.get("user_phone") or row.get("user_email") or row.get("session_id") or "")
        # Memory fetch is still 1-per-convo, but we only do it for 5 now
