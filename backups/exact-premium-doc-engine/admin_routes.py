...
def create_invoice():
    if request.method == "POST":
        document_type = request.form.get("document_type", "Invoice").strip() or "Invoice"
        amount = _safe_float(request.form.get("amount"))
        discount = _safe_float(request.form.get("discount"))
        vat = _safe_float(request.form.get("vat"), 15)

        discounted = amount - (amount * (discount / 100))
        final_total = discounted + (discounted * (vat / 100))

        invoice = {
            "document_type": document_type,
            "invoice_code": _tarasi_invoice_code(document_type),
            ...
            "prepared_by": request.form.get("prepared_by", "Tarasi Admin").strip(),
        ...
        invoice["share_text"] = (
            f"{invoice['document_type']} {invoice['invoice_code']} - "
            f"Tarasi Shuttle and Transfer Services CC. "
            f"Client: {invoice['client_name']}. "
            f"Total: N$ {invoice['final_total']:.2f}. "
            f"Bank: FNB, Account: 64289981259, Branch: 282273."
        )

        invoice["saved_to_supabase"] = _save_invoice_record(invoice)
