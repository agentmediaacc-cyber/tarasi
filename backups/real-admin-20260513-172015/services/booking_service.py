...
# -------------------------------------------------------------------
# Real booking page context for premium /book page
# Added safely without replacing existing booking logic
# -------------------------------------------------------------------
import os
import requests

def _tarasi_supabase_headers():
    key = (
...
def _tarasi_fetch_table(table):
    ...
    try:
        res = requests.get(
            ...
            headers=_tarasi_supabase_headers(),
...
def _tarasi_icon(text):
    text = str(text or "").lower()
    if "airport" in text:
        return "✈"
    if "vip" in text or "executive" in text:
        return "★"
    if "school" in text:
        return "🎒"
    if "tour" in text or "safari" in text or "desert" in text:
        return "🏜"
...
def get_booking_context():
    package_table = os.getenv("TARASI_PACKAGES_TABLE", "tarasi_service_packages")
    route_table = os.getenv("TARASI_ROUTES_TABLE", "tarasi_routes")
    fleet_table = os.getenv("TARASI_FLEET_TABLE", "tarasi_fleet")

    packages_raw = _tarasi_fetch_table(package_table)
    routes_raw = _tarasi_fetch_table(route_table)
    fleet_raw = _tarasi_fetch_table(fleet_table)

    packages = []
    for row in packages_raw:
        package_type = row.get("package_type") or row.get("service_type") or row.get("category") or row.get("name") or "Transport"
        packages.append({
            "id": row.get("id") or "",
            "name": row.get("name") or row.get("title") or package_type,
            "package_type": package_type,
            "description": row.get("description") or row.get("summary") or "Premium Tarasi transport service configured by admin.",
            "price": _tarasi_money(row.get("starting_price") or row.get("price") or row.get("base_price") or row.get("amount")),
            "duration": row.get("duration") or row.get("duration_text") or "Flexible",
            "best_for": row.get("best_for") or row.get("suitability") or "Namibia travel",
            "icon": row.get("icon") or _tarasi_icon(package_type),
            "badge": row.get("badge") or row.get("label") or "Premium",
        })

    routes = []
    for row in routes_raw:
        pickup = row.get("pickup") or row.get("origin") or row.get("from_location") or "Pickup not set"
        dropoff = row.get("dropoff") or row.get("destination") or row.get("to_location") or "Drop-off not set"
        routes.append({
            "id": row.get("id") or "",
            "name": row.get("name") or row.get("route_name") or f"{pickup} to {dropoff}",
            "pickup": pickup,
            "dropoff": dropoff,
            "category": row.get("category") or row.get("route_type") or "Route",
            "price": _tarasi_money(row.get("starting_price") or row.get("price") or row.get("base_price") or row.get("amount")),
            "distance_km": row.get("distance_km") or row.get("distance") or 0,
            "duration": row.get("duration") or row.get("duration_text") or "Not set",
            "vehicle": row.get("recommended_vehicle") or row.get("vehicle") or row.get("vehicle_type") or "Admin to assign",
        })

    fleet = []
    for row in fleet_raw:
        fleet.append({
            "id": row.get("id") or "",
            "name": row.get("name") or row.get("vehicle_name") or row.get("model") or "Tarasi Vehicle",
            "vehicle_type": row.get("vehicle_type") or row.get("type") or "Premium Vehicle",
            "seats": row.get("seats") or row.get("capacity") or "Not set",
            "luggage": row.get("luggage") or row.get("luggage_capacity") or "Not set",
            "comfort": row.get("comfort") or row.get("comfort_level") or "Premium",
            "price": _tarasi_money(row.get("starting_price") or row.get("price") or row.get("base_price")),
        })
    ...
    return {
        ...
        "source": "supabase" if os.getenv("SUPABASE_URL") else "not_configured",
    }
